...
export function useMingleSession(userId: string | null) {
  ...
  const setupPeer = useCallback((sessionId: string, isOfferer: boolean) => {
  ...
  const subscribeToSignalChannel = useCallback((sessionId: string) => {
    ...
    channel
      .on("broadcast", { event: "offer" }, async ({ payload }) => {
        ...
        await channel.send({ type: "broadcast", event: "answer", payload: { from: userId, sdp: answer } });
      })
      ...
      .on("broadcast", { event: "ready" }, async ({ payload }) => {
        ...
        if (isOffererRef.current && pcRef.current) {
          ...
          await channel.send({ type: "broadcast", event: "offer", payload: { from: userId, sdp: offer } });
        }
      })
  ...
  const skip = useCallback(async () => {
    if (channelRef.current) {
      try { await channelRef.current.send({ type: "broadcast", event: "bye", payload: { from: userId } }); } catch {}
    }
    await cleanup(true);
    setStatus("idle");
  ...
  return {
    status, session, partnerId, partnerUsername, messages,
    localStream, remoteStream, error,
    findPartner, sendMessage, skip, stop,
  };
}